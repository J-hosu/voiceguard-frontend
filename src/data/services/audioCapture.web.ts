...
class WebAudioCapture implements AudioCaptureService {
  ...
  private async setupGraph(Ctx: any, rate: number): Promise<boolean> {
    try {
      ...
      const chunkSamples = Math.floor(actualRate * CHUNK_SECONDS);
  ...
  /** 누적 버퍼에서 chunkSamples만큼 잘라 WAV 청크로 방출 */
  private flush(chunkSamples: number, rate: number) {
    ...
    const rest = merged.subarray(chunkSamples);
    this.frames = rest.length ? [rest.slice(0)] : [];
    this.frameLen = rest.length;
  ...
  /** 아직 3초를 못 채운 남은 버퍼를 한 청크로 즉시 방출(종료 직전 마지막 발화 보존). */
  flushFinal() {
